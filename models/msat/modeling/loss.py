...
def bce_rescale_loss(config, logits_text, logits_visual, logits_iou, iou_mask_map, gt_maps, gt_times, word_label,
    ...
    gt_p = gt_maps[:, :3, :]
    loss = F.binary_cross_entropy_with_logits(logits_visual[:, :, :], gt_p,
    ...
    s_e_time = (s_e_idx + logits_iou[:, :2, :, :]).clone().detach() if config.w_reg_loss > 0 else s_e_idx
    ...
    loss_dict = dict(stage_loss=stage_loss, reg_loss=reg_loss, iou_loss=iou_loss, mask_loss=mask_loss, loss=loss_value)
